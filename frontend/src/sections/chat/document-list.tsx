...
export function DocumentList({ documents, onDelete, onUpload, sx, ...other }: Props) {
		...
		return (
		  ...
		  >
			<CardHeader
			  ...
			  subheader={`${safeDocuments.length} file${safeDocuments.length !== 1 ? 's' : ''}`}
			  action={
				<Button
				  size="small"
				  variant="outlined"
...
>
  <List disablePadding>
    {safeDocuments.map((doc, index) => {
      const displayName = doc.filename?.split('/').pop() || doc.filename || 'Unknown file';
      ...
      return (
        <Box key={`${doc.filename}-${index}`}>
          ...
          >
            ...
            >
              <Iconify icon={'vscode-icons:file-type-pdf2' as any} width={24} />
            </Box>
            <ListItemText
              primary={displayName}
